...
| 동선 목록에서 부스 위치를 지도로 확인하는가? | `route_booth_highlighted` | `surface`, `position`, `booth_id` |
| 지도 위 동선 표시가 사용되는가? | `route_map_visibility_changed` (`visible = true`) | `saved_count`, `surface` |
| 동선을 실제 방문 체크에 활용하는가? | `route_booth_visited_changed` | `visited`, `visited_count`, `saved_count`, `surface` |
...
5. 방문 완료율: `route_booth_visited_changed(visited = true)`의 `visited_count / saved_count`
6. 기록 기능 이용자: `booth_memo_saved` 또는 `booth_photo_changed` 고유 사용자 추이
